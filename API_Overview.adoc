...
* ASSETS type contains asset, query, and search APIs. DII asset attributes and metrics are sometimes referred to as “odata”. The APIs for these are typically accessed through /rest/v1/asset or /rest/v1/query. See the <<api-documentation-swagger,API Swagger documentation>> for more information.
...
* DATA INGESTION type is used to retrieve and manage ingestion data and custom metrics, such as from Telegraf agents. These are sometimes referred to as “datalake” or “lake” metrics. The APIs for these typically accessed through /rest/v1/lake. See the <<api-documentation-swagger,API Swagger documentation>> for more information.
...
Note that the API types to which a user has access depend also on the link:concept_user_roles.html[User Role] they have in each Data Infrastructure Insights feature set (Observability, Workload Security, Reporting).
...
* Follow the steps for link:task_config_telegraf_agent_k8s.html#manual-upgrades[Manual Upgrade], selecting the new API token.

Note: Customers managing their NetApp Kubernetes Monitoring Operator with a configuration management tool, such as Kustomize, can follow the same steps to generate and download an updated set of YAMLs to push to their repository.
