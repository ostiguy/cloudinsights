...
summary: Create custom queries to search, filter, and analyze your infrastructure assets and metrics with granular control, flexible grouping, and customizable results display.
...
[.lead]
Create powerful queries to search your infrastructure assets at a granular level with flexible filtering, grouping, and sorting capabilities. Build targeted searches that help you identify specific assets, analyze performance metrics, and organize results exactly how you need them. 
...
When filtering for a Boolean value, you may be presented with the following choices on which to filter:
...
* *None*: Returns only results where the value has not been set at all. Also referred to as "Null" values.
