...
<<<<<<< HEAD
summary: Protect your environment by automatically or manually blocking user access when attacks are detected, with configurable time periods and support for SMB and NFS protocols.
=======
summary: Stop detected attacks immediately by restricting compromised user access automatically or manually, preventing further damage to file systems across multiple storage volumes.
...
[.lead]
<<<<<<< HEAD
Respond immediately to security threats with Workload Security's user access blocking capability. When attacks are detected, automatically block user access through Automated Response Policies or manually restrict access from alert and user detail pages to stop attacks and protect your file systems.
