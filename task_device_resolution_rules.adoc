...
. Click *Observability > Enrich > Device Resolution*
...
** hba3
+
* Rules using Regular expressions require a regular expression, the format, and case sensitivity selection.
...
To prevent a rule from running, choose "Pause" from the rule's right-side menu, or select multiple rules and choose "Pause" from the "Bulk Actions" drop-down. You can resume rule running in the same way.
...
Alternatively, you can eliminate the guesswork with AI-powered device identification to create rules quickly and easily. Simply run the AI-Assisted Rule Generator to analyze your environment and suggest Device Resolution rules, helping you quickly identify and manage more devices. 

=== Get Started

To start using AI to generate Resolution Rules, an administrator must first enable AI Generated Rules. 

image:ahr_ai_get_started.png[Get Started generating AI Rules, width=400]

The AI assistant will tell you what percentage of your fibre channel devices are unidentified, and prompt you to allow it to generate rules.  Select "Let's Go!".

image:ahr_ai_lets_go.png[Select "Let's Go" to generate rules, width=400]

You are prompted to confirm your domain names before enabling AI-generated rules. You must include all of your domains in order for effective identification.

=== Check Progress

Periodically, DII will create a notification when additional devices are available for possible identification. If you see a green dot on the Bell icon at the top right of the page, select it to view the notification. 

image:ahr_ai_notification.png[notification bell, width=400]

Use the Bell icon to also check on the status of rule generation, which can take from a few seconds to several minutes, depending on the complexity of your environment.

Rules generated by AI will display an "AI Generated" badge in the list.

image:ahr_ai_badge.png[ai-generated rule badge, width=400]

=== Resume and Go

AI-generated rules are in _Paused_ state by default. You must review each rule and _Resume_ it before it will identify devices.
