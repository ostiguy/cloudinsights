...
|SVM type / subtype |Data SVMs only (no infinite volumes). Supported subtypes: default, sync_source, sync_destination. DP-destination SVMs are skipped for audit by design.
...
|Snapshots |volume snapshot: all (scope to cloudsecure_* where possible) |Any
