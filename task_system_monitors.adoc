...
summary: Review system-defined monitors for metric, log, attribute, and data collection conditions.
---
...
System-defined monitors are comprised of pre-defined metrics or attributes and conditions, as well as default descriptions and corrective actions, which can not be modified. You _can_ modify the notification recipient list for system-defined monitors. To view the metrics or attributes, conditions, description and corrective actions, or to modify the recipient list, open a system-defined monitor group and click the monitor name in the list.
...
* *CVO Infrastructure* and *CVO Workload Examples* include corresponding monitors for Cloud Volumes ONTAP.
* Default state varies by monitor. Refer to the tables below and review a monitor before enabling notifications.
...
|Shelf Fan Failed|CRITICAL|The indicated cooling fan or fan module of the shelf has failed. The disks in the shelf might not receive enough cooling airflow, which might result in disk failure.|Perform the following corrective actions:…Verify that the fan module is fully seated and secured.
...
|NVMe-oF License Grace Period Active|WARNING|This event occurs on a daily basis when the NVMe over Fabrics (NVMe-oF) protocol is in use and the grace period of the license is active. The NVMe-oF functionality requires a license after the license grace period expires. NVMe-oF functionality is disabled when the license grace period is over. |Contact your sales representative to obtain an NVMe-oF license, and add it to the cluster, or remove all instances of NVMe-oF configuration from the cluster. 
|NVMe-oF License Grace Period Expired|WARNING|The NVMe over Fabrics (NVMe-oF) license grace period is over and the NVMe-oF functionality is disabled.|Contact your sales representative to obtain an NVMe-oF license, and add it to the cluster.
|NVMe-oF License Grace Period Start|WARNING|The NVMe over Fabrics (NVMe-oF) configuration was detected during the upgrade to ONTAP 9.5 software. NVMe-oF functionality requires a license after the license grace period expires.|Contact your sales representative to obtain an NVMe-oF license, and add it to the cluster.
...
|Kubernetes Persistent Volume Reclamation Failed
...
|Volume Discovered|Informational|This message occurs when an Internal Volume is discovered.
|Volume Modified|Informational|This message occurs when an Internal Volume is modified.
|Node Discovered|Informational|This message occurs when an Storage Node is discovered.
|Node Removed|Informational|This message occurs when an Storage Node is removed.
...
Attribute monitors evaluate configuration attributes reported for an object. Unlike metric monitors, they do not compare a time-series metric with a numeric threshold. Each condition below generates a _Warning_ alert when the reported attribute matches a noncompliant or unavailable state.

The previous standalone ONTAP Essentials security metric monitors have been replaced by the consolidated *ONTAP Cluster Misconfigured* and *ONTAP Storage Misconfigured* attribute monitors. The corresponding CVO monitors apply the same checks to Cloud Volumes ONTAP.

==== ONTAP Attribute Monitors

|===
|Monitor Name|Group|Default State|Conditions|Corrective Action

|ONTAP Cluster Misconfigured
|ONTAP Infrastructure
|Active
|Alerts when one or more of these 12 security conditions is detected:

* AutoSupport HTTPS is not configured
* Web HTTP is enabled
* SSH is not secured
* The login banner is disabled
* Cluster peer communication is not encrypted
* The certificate issuer is self-signed
* The default local administrator is enabled
* FIPS is disabled
* Log forwarding is not encrypted
* MD5 password hashing is enabled
* Remote Shell (RSH) is enabled
* Telnet is enabled
|Review the affected attributes in the alert. Disable insecure protocols and password hashing, secure SSH and peer communication, enable the required security controls, and replace self-signed certificates according to your organization's security policy.

|ONTAP Storage Misconfigured
|ONTAP Infrastructure
|Active
|Alerts when an ONTAP Storage VM has audit logging disabled, SSH is not secured, or its login banner is disabled.
|Enable Storage VM audit logging, secure SSH, and configure an appropriate login banner.

|ONTAP Cluster Lacks Spare Disks
|ONTAP Infrastructure
|Active
|Alerts when the cluster reports that spare disks are low.
|Review spare disks by node and add compatible spare capacity to maintain data protection.

|ONTAP Storage Service Unavailable
|ONTAP Workload Examples
|Active
|Alerts when an enabled FCP or iSCSI service is unavailable on an ONTAP Storage VM.
|Verify Storage VM, protocol, and data LIF status. Review recent ONTAP health and event messages, and restore the affected SAN service.

|ONTAP Node Misconfigured
|ONTAP Workload Examples
|Paused
|Alerts when Cisco Discovery Protocol (CDP) is disabled on an ONTAP node.
|Verify the node's CDP setting and network port status, then enable CDP where it is required for network discovery and topology monitoring.
|===

==== Cloud Volumes ONTAP Attribute Monitors

|===
|Monitor Name|Group|Default State|Conditions|Corrective Action

|CVO Cluster Misconfigured
|CVO Infrastructure
|Active
|Applies the cluster security configuration checks described for *ONTAP Cluster Misconfigured* to Cloud Volumes ONTAP.
|Review the affected attributes and correct the reported insecure or noncompliant cluster settings.

|CVO Storage Misconfigured
|CVO Infrastructure
|Active
|Alerts when a Cloud Volumes ONTAP Storage VM has audit logging disabled, SSH is not secured, or its login banner is disabled.
|Enable Storage VM audit logging, secure SSH, and configure an appropriate login banner.

|CVO Node Misconfigured
|CVO Workload Examples
|Paused
|Alerts when CDP is disabled on a Cloud Volumes ONTAP node.
|Verify the node's CDP and network settings, then enable CDP where required.
|===

==== NTP Configuration Monitors

The following NTP monitors remain available as standalone system-defined monitors for ONTAP and Cloud Volumes ONTAP.

|===
|Monitor Name|Platform|Threshold|Monitor Description|Corrective Action

|No NTP Servers are Configured
|ONTAP
|Warning @ < 1
|The cluster has no configured NTP servers. For redundancy and optimum service, NetApp recommends associating at least three NTP servers with the cluster.
|Associate an NTP server with the cluster by using `cluster time-service ntp server create -server <ntp_server>`.

|NTP Server Count is Low
|ONTAP
|Warning @ < 3
|The cluster has fewer than three configured NTP servers.
|Associate additional NTP servers with the cluster by using `cluster time-service ntp server create -server <ntp_server>`.

|CVO No NTP Servers are Configured
|Cloud Volumes ONTAP
|Warning @ < 1
|The cluster has no configured NTP servers.
|Associate an NTP server with the cluster by using `cluster time-service ntp server create -server <ntp_server>`.

|CVO NTP Server Count is Low
|Cloud Volumes ONTAP
|Warning @ < 3
|The cluster has fewer than three configured NTP servers.
|Associate additional NTP servers with the cluster by using `cluster time-service ntp server create -server <ntp_server>`.
...
|MetroCluster FC Initiator Link Down|CRITICAL|The FC initiator adapter is at fault.|1. Ensure that the FC initiator link has not been tampered with.
...
|MetroCluster FC-VI Interconnect Link Down|CRITICAL|The physical link on the FC-VI port is offline.|1. Ensure that the FC-VI link has not been tampered with.
